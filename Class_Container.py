...
# NOTE: that inheritance at this point only does the visual work of making children class have parent = their parent
# it does not for example, make this child class inherit a attribute or method unique to the parent...
...
Does the same thing as the inherit function in misc_functions.py
    """
    line = line.split(":")
    family, family_attr, family_methods = line[0].split(
        ">"), line[1].split(">"), line[2].split(">")
    ...
    name, attr, methods = ">".join(family), ">".join(
